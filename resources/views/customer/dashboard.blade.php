    ...
    <div id="katalog" class="max-w-6xl mx-auto pt-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
            @foreach($categories as $category)
                <!-- Card Kategori -->
                <div onclick="openModal('modal-{{ Str::slug($category->name) }}')" class="cursor-pointer group relative flex flex-col overflow-hidden rounded-3xl bg-white shadow-sm border border-rose-50 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-rose-200 text-left w-full focus:outline-none focus:ring-4 focus:ring-rose-200">
                    <div class="aspect-square w-full overflow-hidden bg-rose-50 relative">
                        <img src="{{ $category->image }}" alt="{{ $category->name }}" class="h-full w-full object-cover object-center transition-transform duration-700 group-hover:scale-110" />
                        ...
                        <div class="absolute bottom-0 left-0 w-full p-4 text-white">
                            <span class="inline-block rounded-md bg-rose-500/90 backdrop-blur-md px-2 py-0.5 text-[10px] font-bold text-white mb-2 shadow-sm">
                                {{ count($category->services) }} Layanan
                            </span>
                ...
                <!-- Modal List Layanan untuk Kategori ini -->
                <div id="modal-{{ Str::slug($category->name) }}" class="fixed inset-0 z-50 hidden bg-gray-900/60 backdrop-blur-md transition-opacity" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
                        <div class="relative transform overflow-hidden rounded-[2.5rem] bg-white text-left shadow-2xl transition-all sm:my-8 w-full sm:max-w-3xl flex flex-col max-h-[85vh] ring-1 ring-rose-100">
                            ...
                            <div class="relative h-28 sm:h-32 w-full shrink-0">
                                ...
                                <!-- Tombol Close -->
                                <button onclick="closeModal('modal-{{ Str::slug($category->name) }}')" class="absolute top-4 right-4 h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center rounded-full bg-white/20 text-white backdrop-blur-md hover:bg-rose-500 hover:text-white transition-all">
                                ...
                                <div class="absolute bottom-4 left-6 sm:bottom-6 sm:left-8">
                                    <h3 class="text-2xl sm:text-3xl font-extrabold font-serif text-white drop-shadow-md">{{ $category->name }}</h3>
                                </div>
                            </div>
                            ...
                            <div class="shrink-0 bg-white p-5 border-t border-rose-100 flex flex-col sm:flex-row justify-between items-center gap-4 rounded-b-[2.5rem]">
                                ...
                                <a href="{{ route('customer.reservations.create', ['category' => $category->name]) }}" 
